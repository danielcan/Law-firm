        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                        $query = "SELECT * FROM especialidad WHERE estado = 'activo'";
                        $query_run = mysqli_query($con, $query);
                        if (mysqli_num_rows($query_run) > 0) {
                            foreach ($query_run as $esp) {
                        ?>
                                <tr>
                                    <td><?= $esp['IdEsp']; ?></td>
                                    <td><?= $esp['nombre']; ?></td>
                                    <td>
                                        <a href="menu-abo.php?controlador=especialidad-edit&idesp=<?= $esp['IdEsp']; ?>" class="btn btn-success btn-sm">Editar</a>
                                        <form action="codeesp.php" method="POST" class="d-inline">
                                            <button type="submit" name="delete_espe" value="<?= $esp['IdEsp']; ?>" class="btn btn-danger btn-sm">Eliminar</button>
                                        </form>
                                    </td>
                                </tr>
                        <?php
