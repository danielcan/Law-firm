          ...
          <div class="card">
              <div class="card-header">
                  <h4>Agrega una nueva asignación de esta cita a un abogado
                      <a href="menu-abo.php?controlador=asignacioncita" class="btn btn-danger float-end">Regresar</a>
                  </h4>
              </div>
              <div class="card-body">
                  <form action="codeasigna.php" method="POST">
                      ...
                      <?php
                        if (isset($_GET['idAsig'])) {
                            $asig_id = mysqli_real_escape_string($con, $_GET['idAsig']);

                        }

                        ?>

                      <input type="text" name="asig_id" value="<?= $asig_id; ?>" hidden>
                          ...
                          <div>
                              <select name="abog_id" size="1" id="Esp" class="form-control" required="required">
                                  ...
                                  <?php
                                    ...
                                    $consulta = "SELECT abogado.IdAbo ,perfil.primer_nomb, perfil.primer_ape FROM `abogado`,`perfil` WHERE abogado.IdPer = perfil.IdPer";
                      ...
                      <div class="mb-3">
                          <div>
                              <div class="mb-3">
                                  <button type="submit" name="update_asigna" class="btn btn-primary">Agregar la asignación al abogado</button>
                              </div>
                          </div>
                      </div>
                  </form>
